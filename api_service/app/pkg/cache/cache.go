...
type Repository interface {
	...
	// Del deletes an item in the cache by key and returns true or false if a deleting occurred
	Del(key []byte) (affected bool)

	// EntryCount returns the number of items currently in the cache
	EntryCount() (entryCount int64)

	// HitCount is a metric that returns number of items a key was found in the cache
	HitCount() int64

	// MissCount is a metric that returns the number of items a miss occurred in the cache
	MissCount() int64
}
